...
async def test_peer_list_shows_registered_peers(relay_dir):
    ...
    async with app.run_test(headless=True) as pilot:
        ...
        # List now shows only children (related to acting-as parent).
        # "parent" has no "-" so prefix is empty → fallback: all children shown.
...
@pytest.mark.asyncio
async def test_list_shows_only_related_children(relay_dir):
    from claude_relay.core import store
    ...
    async with app.run_test(headless=True) as pilot:
        ...
        peer_widget.refresh_from_store()
        names = [item.peer_name for item in peer_widget.items]
        assert set(names) == {"PLAT-3113-child", "PLAT-3113-slave"}
        assert "other-child" not in names
        assert "PLAT-3113-master" not in names  # parent itself not in list


@pytest.mark.asyncio
async def test_list_falls_back_to_all_children_when_prefix_empty(relay_dir):
    from claude_relay.core import store
    store.register_peer(name="parent", session_id="s1", cwd="/tmp", role="parent")
